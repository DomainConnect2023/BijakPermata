...
export function BuyCancelScreen({ initialFromDate, initialToDate }: Props) {
  ...
  const [items, setItems] = useState<CancelledTransactionItem[]>([]);
  ...
  const loadReport = useCallback(async (from: Date, to: Date) => {
    ...
    try {
      const data = await fetchBuyCancel(from, to);
      setItems(data);
    } catch (error) {
      setErrorMessage(
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.centerContent}>
              ...
              <Text style={styles.emptyText}>No cancelled purchases</Text>
          ...
          renderItem={({ item }) => (
            <View style={styles.transactionCard}>
              ...
              {(item.customer || item.user) && (
                <View style={styles.metaRow}>
                  {item.user && (
                    <Text style={styles.metaText}>By: {item.user}</Text>
                  )}
                  {item.customer && (
              ...
              {item.remCancel && item.remCancel.trim() !== "" && (
