...
export function DailyStockReportScreen() {
  ...
  const loadReport = useCallback(async (forDate: Date) => {
    ...
    try {
      const data = await fetchDailyStock(forDate);
      setItems(data);
    } catch (error) {
      setErrorMessage(
  ...
  const renderCurrencyItem = ({ item }: { item: DailyStockItem }) => (
    <View style={styles.currencyCard}>
      <View style={styles.cardHeader}>
        <View style={styles.currencyBadge}>
          <Text style={styles.currencyCode}>{item.currency.trim()}</Text>
        </View>
        <View style={styles.balanceContainer}>
          <Text style={styles.balanceLabel}>Closing Balance</Text>
          <Text style={styles.balanceValue}>
            RM {formatAmount(item.balRM)}
          </Text>
          <Text style={styles.balanceSubtext}>
            {formatAmount(item.balance)} {item.currency.trim()}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.cardBody}>
        ...
        <View style={styles.cardFooter}>
          <View style={styles.footerItem}>
            ...
            <Text style={styles.footerLabel}>Opening</Text>
            <Text style={styles.footerValue}>
              RM {formatAmount(item.obRM)}
            </Text>
          </View>
          <View style={styles.footerDivider} />
          <View style={styles.footerItem}>
            <Ionicons name="pricetag-outline" size={14} color="#6B7280" />
            <Text style={styles.footerLabel}>Sale Cost</Text>
            <Text style={styles.footerValue}>
              RM {formatAmount(item.saleCost)}
            </Text>
          </View>
        </View>
      </View>
    ...
    <View style={styles.container}>
      ...
      {!loading && !errorMessage && items.length > 0 && (
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryItem}>
            <Text style={styles.summaryLabel}>Total Closing Balance</Text>
            <Text
              style={[
                styles.summaryValue,
                totalBalanceRM >= 0 ? styles.summaryIn : styles.summaryOut,
              ]}
            >
              RM {formatAmount(totalBalanceRM)}
            </Text>
          </View>
        </View>
      )}
      ...
      {loading ? (
        ...
        <View style={styles.centerContent}>
          ...
          <Pressable onPress={() => loadReport(date)} style={styles.retryButton}>
